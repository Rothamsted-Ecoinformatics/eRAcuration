<label class="block">
    <table class="min-w-full mt-5">
        <thead class="">
            <tr>
                <th class="text-left">
                    <span class="p-2 text-lg font-semibold text-gray-700">or Authoring organisations</span>
                </th>
            ...
            <tr class="bg-slate-300">
                <td class="w-5/6 py-3 px-4 text-left">
                    <select class="form-select mt-1 block w-full rounded-md" wire:model="authorOrg">
                        ...
                        @foreach ($filteredAuthorOrgs as $filteredAuthorOrg)
                            <option value="{{ $filteredAuthorOrg->id }}">
                                {{ $filteredAuthorOrg->name }} ({{ $filteredAuthorOrg->abbreviation }})
                            </option>
                        @endforeach
                    </select>
                </td>
                <td class="w-1/6 py-3 px-4 text-right">
                    <button
                        ...
                        wire:click.prevent="addAuthorOrg">Add</button>
                </td>
            </tr>
